...
contaminant_data <- data_cleaned %>%
  filter(Water_Object %in% selected_objects$Water_Object) %>%  # Filter for selected objects
  select(Water_Object, all_of(pollutants)) %>%  # Select relevant columns
  pivot_longer(
...
**Null Hypothesis (H0)**: The concentrations of water quality components (e.g., dissolved oxygen, COD, chemical contaminants) are consistent across all water objects.
**Alternative Hypothesis (H1)**: The concentrations of water quality components vary significantly across different water objects.
...
Null Hypothesis (H₀): There are no temporal trends in the concentrations of key water quality components.
Alternative Hypothesis (H₁): There are significant temporal trends in the concentrations of key water quality components, such as dissolved oxygen and COD.

Hypothesis 3:

Null Hypothesis (H₀): The adjustments made to the data through multiplication factors have no significant impact on the observed trends and patterns.
Alternative Hypothesis (H₁): The adjustments made to the data significantly influence the observed trends and patterns, potentially leading to deviations from actual conditions.

Hypothesis 4:

Null Hypothesis (H₀): The limited number of sources and samples has no significant impact on the representativeness of the dataset.
Alternative Hypothesis (H₁): The limited number of sources and samples affects the representativeness of the dataset, leading to potential biases in water quality assessments.
