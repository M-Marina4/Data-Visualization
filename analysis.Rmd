...
```{r}
threshold <- 1e-10  # Define a threshold for small values
# Replace very small values (absolute values less than threshold) with 0
data_cleaned[columns_to_convert] <- lapply(data_cleaned[columns_to_convert], function(x) {
  if (is.numeric(x)) {
    x[abs(x) < threshold] <- 0
  }
  return(x)
